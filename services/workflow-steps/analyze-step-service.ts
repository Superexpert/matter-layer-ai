...
import { APPRAISAL_REVIEW_RESPONSE_FORMAT, parseAppraisalReviewGeneratorOutput, renderCondemnorAppraisalReview, type AppraisalReviewGeneratorOutput } from "@/workflow-steps/analyze/appraisal-review";
...
function collapsedFactsFromOutput(value: unknown): CollapsedFact[] {
  ...
  if (value.collapsedFacts.length === 0) {
    if (value.profile === "condemnor-appraisal-review") {
      throw new Error("No appraisal could be identified in the selected documents. Select a condemnor appraisal or appraisal addendum and retry extraction.");
    }
...
export async function runAnalyzeStep(input: RunAnalyzeStepInput): Promise<AnalyzeStepOutput> {
  ...
  const generated = await mapBounded(targets, async (generator): Promise<AnalyzeGeneratorResult> => {
    ...
    try {
      ...
      const response = await aiService.generateText({ maxOutputTokens: 8000, messages: analyzeGeneratorMessages({ aggregate: Boolean(config.aggregate), generator, packet }), model: settings.model, responseFormat: config.aggregate ? APPRAISAL_REVIEW_RESPONSE_FORMAT : ANALYZE_WORK_PRODUCT_RESPONSE_FORMAT });
      if (config.aggregate) {
        return { ...generator, completedAt: new Date().toISOString(), resultJson: parseAppraisalReviewGeneratorOutput(response.content), status: "completed" };
      }
  ...
  let artifactIds = completed.flatMap((generator) => generator.artifactId ? [generator.artifactId] : []);
  if (config.aggregate && status === "completed") {
    const existing = await existingGeneratorArtifact({ generatorId: `aggregate:${config.aggregate.renderer}`, stepId: input.step.id, workflowRunId: input.workflowRunId });
    if (existing) {
      artifactIds = [existing.id];
    } else {
      const results = Object.fromEntries(generators.map((generator) => [generator.id, generator.resultJson as AppraisalReviewGeneratorOutput]));
      const content = renderCondemnorAppraisalReview({ packet, results });
      const artifact = await createWorkflowMarkdownArtifact({ content, matterId: input.matterId, metadataJson: { description: "Generated structured appraisal review", generatorId: `aggregate:${config.aggregate.renderer}`, model: settings.model, provider: settings.provider }, stepId: input.step.id, title: config.aggregate.outputName, workflowRunId: input.workflowRunId });
      artifactIds = [artifact.id];
    }
  }
  const output: AnalyzeStepOutput = { artifactIds, completedAt: new Date().toISOString(), generators, inputStepId: config.inputStepId, model: settings.model, providerId: settings.provider, startedAt, status };
