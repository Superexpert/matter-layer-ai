...
export type RunAnalyzeStepInput = {
  aiService?: { generateText: (request: { maxOutputTokens?: number; messages: Array<{ content: string; role: "assistant" | "system" | "user" }>; model?: string }) => Promise<{ content: string; model: string; provider: string }> };
...
export async function runAnalyzeStep(input: RunAnalyzeStepInput): Promise<AnalyzeStepOutput> {
  ...
  const generated = await mapBounded(targets, async (generator): Promise<AnalyzeGeneratorResult> => {
    ...
    try {
      ...
      const response = await aiService.generateText({ maxOutputTokens: 8000, messages: analyzeGeneratorMessages({ generator, packet }), model: settings.model });
      const content = response.content.trim();
      if (!content) throw new Error("Analyze generator returned empty Markdown.");
