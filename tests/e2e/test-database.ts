...
export function assertTestDatabaseUrl(databaseUrl: string | undefined) {
  if (!databaseUrl?.trim()) {
    throw new Error("Playwright tests require DATABASE_URL in .env.test.local.");
  }
  ...
  if (!searchableUrl.includes("test")) {
    throw new Error(
      `Refusing to run Playwright database setup against non-test database "${databaseName}". DATABASE_URL must clearly contain "test".`,
    );
  }
}
